...
def get_reference_data(lr_system_folder: Path, file_name: str = "reference_data.csv") -> FeatureData:
    ...
    return FeatureDataCsvParser(reference_data_file, hypothesis_column="hypothesis").get_instances()


def get_validation_experiment(model_name: str, training_data_path: Path, output_path: Path) -> Path:
    """
    Build and run a validation experiment, returning the folder with the trained model.

    The `lr_system_folder` will be created in `output_path` under the name "model". Any reference data generated by the
    validation, which lir writes to `output_path`, is also copied into this folder so that the model folder is
    self-contained.
    """
    ...
    output_folder = output_path / "model"
    reference_data_file = output_path / "reference_data.csv"
    if reference_data_file.exists():
        shutil.copy(reference_data_file, output_folder / "reference_data.csv")
